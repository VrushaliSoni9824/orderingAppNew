        ...
        results.addSource(apiResponse, new Observer<ApiResponse<RequestObject>>() {
            ...
            public void onChanged(@Nullable final ApiResponse<RequestObject> requestObjectApiResponse) {
                ...
                if (requestObjectApiResponse instanceof ApiResponse.ApiSuccessResponse) {
                    Log.e(TAG, "onChanged: ApiSuccessResponse.");

                    appExecutors.diskIO().execute(() -> {

                        // save the response to the local db
                        saveCallResult((RequestObject) processResponse((ApiResponse.ApiSuccessResponse) requestObjectApiResponse));
                        Log.e("tag"," =  = = catch = =   " + ((ApiResponse.ApiSuccessResponse) requestObjectApiResponse).getBody());
                    ...
                    Log.e(TAG, "onChanged: ApiEmptyResponse");
                    appExecutors.mainThread().execute(() -> results.addSource(loadFromDb(), new Observer<CacheObject>() {
                        @Override
                        public void onChanged(@Nullable CacheObject cacheObject) {
                            setValue(Resource.success(cacheObject));
                        }
                    ...
                    Log.e(TAG, "onChanged: ApiErrorResponse.  = = =  = =   " + ((ApiResponse.ApiErrorResponse) requestObjectApiResponse).getErrorMessage());
